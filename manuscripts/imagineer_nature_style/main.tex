...
The Imagineer main page was reduced in the same cycle. The previous home page attempted to show paper state, WDI state, lane scores, loop health, and signal analysis at once. That analysis now belongs on the profile page and in Progress. The home page now has one purpose: show the current small step, why it was selected only through its source basis, the expected time cost, the freshness timestamp, and a direct link to begin. The current deterministic step is to write one Sarrus design decision in the PhD organization document: constraint, choice, result, and next test. This is more directly coupled to the active constraint than a generic daily log line because the principal-signal gap depends on visible technical ownership.
