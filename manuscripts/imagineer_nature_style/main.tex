...
In the current lock, the weakest live signal is principal signal. The selected action is to lock one FluxCell experiment today: motion target, hardware change, measurement, and first build date. This is not another Sarrus-writing task. Sarrus is the completed technical anchor. The live failure point is that the public Disney case can become past-tense unless there is a measurable current R\&D direction beyond the finished Sarrus paper.
