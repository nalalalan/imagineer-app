...
\centering
\small
\caption{State variables at the current evidence lock. Values are reported from the live system after the profile-freshness and compact-readout update on 9 May 2026.}
\label{tab:state}
...
Reviewer source graph & Thirty-three sources in the latest production run; the explicit source registry now includes Progress, Curtis, Relay Live, Yum, Lily, papers, dashboards, and discovered AO Labs links \\
...
Source count & 33 sources in the latest production run \\
...
Current system work & Profile freshness timestamp, broader AO Labs source graph, compact state surface, continuous paper update \\
\bottomrule
\end{tabularx}
\end{table}
