...
The Imagineer main page was reduced in the same cycle. The previous home page attempted to show paper state, WDI state, lane scores, loop health, and signal analysis at once. That analysis now belongs on the profile page and in Progress. The home page now has one purpose: show the current small step, why it was selected only through its source basis, the expected time cost, the freshness timestamp, and a direct link to begin. The current deterministic step is to set the next FluxCell test by writing the motion target, hardware change, and pass/fail metric. This replaces the earlier Sarrus-design-decision step because the Sarrus paper is now treated as the locked technical anchor; the active principal-signal gap is current technical direction beyond that completed record.
